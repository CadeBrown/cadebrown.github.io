...
We can then parse the name, integers, and operators by consuming the longest chain of characters in a row that match the rule for it (and we'll ignore comments):
...
The code (in my opinion) largely speaks for itself. You keep consuming characters while they fit the current token type, and for constants you just check if the string starts with them, and emit a token if so (then, you use `ADV()` the length of the string to skip over it). 
...
I'm going to go ahead and define my operators, and a bunch of random grammar symbols. These are pretty straightforward if you understand the above, with a few exceptions. For example, you need to put operators that contain other operators before those other operators, or else you'll get unexpected results.

...
Here's the diagram of our compiler:

![assets/img/kata-ckc-arch.svg](assets/img/kata-ckc-arch.svg)

Before we begin writing any individual piece, we'll include some code that will make it easy to report compiler errors and some common utilities:
...
Let's put the definitions for these functions in the file `main.cc`:
...
We also define a `Token` structure that will be generated by our first stage, and can be referenced by errors so they can print out the related lines in the source code.
...
This will skip all whitespace, but throw an error on anything else. Changing `main` function (in `main.cc`, but remember to put this function OUTSIDE of the `namespace kc` block):
...
We can then parse the name, integers, and operators by consuming the longest chain of characters in a row that match the rule for it (and we'll ignore comments, which are just like C and C++ style comments):
...
The code (in my opinion) largely speaks for itself. You keep consuming characters while they fit the current token type, and for constants you just check if the string starts with them, and emit a token if so (then, you use `ADV()` the length of the string to skip over it). For comments, you skip all the characters that are part of the comment, but don't emit any token for it.
...
I'm going to go ahead and define my operators, and a bunch of random grammar symbols. These are pretty straightforward if you understand the above, with a few exceptions. 

For example, you need to put operators that contain other operators before those other operators, or else you'll get unexpected results. I'm going to have the operators `++`, as well as `+`, so I need to put `++` before `+`. Otherwise, `++` will parse as 2 `+` operators instead of a single `++` operator. Here's the full `while` loop:

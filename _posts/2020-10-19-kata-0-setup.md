...
Let's learn how to implement a system programming language. This series will cover the design (including documentation, standardization, etc.), as well as the practical implementation. The first part will just be defining some terms and getting a project set up.
...
Kata's purpose is to be a system programming language, as well as have support for HPC (High Performance Computing) applications and libraries. It will also support modules as packages, which will make dependency management easier (and avoid spillage into a global namespace). Kata's metaprogramming will also be a large way to set it apart from C/C++, as I want to add more type inspection capabilities, as well as a template system.
...
We'll wait on this a bit, but just be thinking of how you'd like to structure it.
...
The `ckc/` will hold a makefile as well as the source for the initial compiler.
...
LIBS             += $(shell $(LLVM_CONFIG) --libs --ldflags core native) -lm -ll
...
g++ ... main.o types.o ... -lLLVM-9 -lm -ll -o ckc
$ ./ckc
usage: ./ckc [file]
$ ./ckc src.kata
...
(the outputs may not exactly match, since this will depend on your LLVM implementation)
